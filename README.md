...
## # Quantitative Modelling of UFC Outcomes

This project is an end-to-end machine learning pipeline designed to predict binary fight outcomes (Win/Loss) in the UFC. The model's predictive power is built on a foundation of complex, time-series feature engineering, including a custom-built chronological Elo rating system and temporally-decayed performance statistics.
...
* Cleaned and standardized all data, including:
    * Converting "n of m" string statistics (e.g., "50 of 100") into numerical `Landed` and `Attempted` columns.
    * Parsing time formats (e.g., "4:15") into total fight time in minutes.
    * Standardizing categorical text features (e.g., `Method`, `Stance`).
* Addressed data sparsity by implementing **Missingness Indicator Flags** (e.g., `Reach_missing`) and using vectorized **median imputation** for physical attributes.
...
Feature importance analysis confirmed that the custom-engineered features (`Elo_Diff`, `Win_Rate_Diff`, `SSpm_Diff`) were the most significant predictors in the final model.
